...
function add_medicine(uint mid, uint m_id,string memory name,string memory ty,string memory a,string memory cn,string memory n,
	string memory ul,string memory se, string memory s) public{
	medicine memory m1=medicine(mid,m_id,name,ty,a,cn,n,ul,se,s);
	m.push(m1);
}
...
function add_stock(uint sid,uint mid,uint m_id,uint did,uint pid,string memory stock,string memory mfg,string memory date) public{
	stocks memory s1=stocks(sid,mid,m_id,did,pid,stock,mfg,date);
	stk.push(s1);
